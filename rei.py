...
if df_after_22.empty:
    ...
    if len(df_after_6.columns) >= 6:
        ...
        # データが十分にあるか確認
        if len(skin_temp_6) >= 16 and len(core_temp) >= 16:
            # 過去10行と現在6行の平均を計算（皮膚温度）
            past_skin_avg1 = skin_temp_6.iloc[-70:-60].mean()
            past_skin_avg2 = skin_temp_6.iloc[-40:-30].mean()
            current_skin_avg = skin_temp_6.iloc[-10:].mean()

            # 過去10行と現在6行の平均を計算（深部体温）
            past_core_avg1 = core_temp_6.iloc[-70:-60].mean()
            past_core_avg2 = core_temp_6.iloc[-40:-30].mean()
            current_core_avg = core_temp_6.iloc[-10:].mean()
