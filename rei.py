...
if df_after_22.empty:
    ...
    if len(df_after_22.columns) >= 6:
        ...
        if len(skin_temp) >= 16 and len(core_temp) >= 16:
            ...
            past_skin_avg1 = skin_temp.iloc[-35:-25].mean()
            past_skin_avg2 = skin_temp.iloc[-20:-10].mean()
            current_skin_avg = skin_temp.iloc[-10:].mean()

            # 過去10行と現在6行の平均を計算（深部体温）
            past_core_avg1 = core_temp.iloc[-35:-25].mean()
            past_core_avg2 = core_temp.iloc[-20:-10].mean()
